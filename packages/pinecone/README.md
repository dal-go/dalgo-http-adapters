...
Pinecone vector values are not exposed as DALgo document fields. This adapter deliberately requests only metadata for reads; vectors inserted outside the adapter without object metadata are rejected rather than decoded ambiguously. Vector IDs are restricted to printable ASCII with no spaces, matching fetch endpoint constraints. A one-record `set` requires Pinecone to acknowledge exactly one upserted record.
