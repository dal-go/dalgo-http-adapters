...
const database = new PineconeDatabase({
  ...
  namespace: "products",
  vectorForWrite: (metadata) => embed(metadata.title),
  headers: () => ({ "Api-Key": tokenFromTrustedBroker() }),
});
```

Use the unique index host supplied by Pinecone, not the control-plane API host. DALgo record data is stored in Pinecone vector `metadata`; `vectorForWrite` is required because Pinecone upsert requires a vector. `set` uses Pinecone upsert, so it intentionally replaces the metadata and vector for that ID.
...
Pinecone vector values are not exposed as DALgo document fields. This adapter deliberately requests only metadata for reads; vectors inserted outside the adapter without object metadata are rejected rather than decoded ambiguously.
