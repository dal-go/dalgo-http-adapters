...
describe("AppwriteDatabase", () => {
  ...
  it("compiles bounded rows queries", async () => {
    ...
    expect(url).toContain("queries%5B%5D=equal%28%22done%22%2C%5Bfalse%5D%29"); expect(url).toContain("orderDesc"); expect(url).toContain("limit%285%29"); expect(url).toContain("offset%282%29");
  });
